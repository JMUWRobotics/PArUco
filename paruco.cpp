...
static void tagCircleEstimates(const std::vector<cv::Point2f> &corners, float scale, std::array<std::optional<cv::Point2f>, 12> &out) {    
...
void detect(const cv::Mat &image, tbb::concurrent_vector<Detection> &detections, const Params &params) {
    ...
    std::sort(blobs.begin(), blobs.end(), [](const cv::KeyPoint &a, const cv::KeyPoint &b) {
    ...
    cv::parallel_for_(cv::Range(0, arucoCorners.size()), [&](const cv::Range &range) {
        ...
        for (int i = range.start; i < range.end; ++i) {
            ...
            tagCircleEstimates(dect.arucoCorners, params.tagExpandScale, dect.circleCenters);
            ...
            for (size_t i = 0; i < dect.circleCenters.size(); ++i) {
                ...
                for (const auto &blob : blobs) {
                    if (float distance2 = cv::normL2Sqr<float>(blob.pt - *center);
                ...
                if (params.maxIdentificationDistancePixels.has_value() &&
                    params.maxIdentificationDistancePixels.value() < std::sqrt(bestDistance2)) 
                {
                    center = std::nullopt;
                    continue;
                }
                ...
                if (auto it = usedBlobs.find(closestKeyPoint->hash()); it != usedBlobs.end()) {
                    ...
                    usedBlobs[closestKeyPoint->hash()] = std::move(associatedBlob);
                    center = closestKeyPoint->pt;
                }

                if (params.refineParams.has_value()) {
                    const auto &refineParams = params.refineParams.value();
                    const float radius = 0.5f * closestKeyPoint->size * refineParams.blobBboxScale;
                    const cv::Point ul {
                    ...
                    if ((blobNeighborhoodRoi & imgRect) != blobNeighborhoodRoi) {
                        center = std::nullopt;
                        continue;
                    }
                    ...
                    if (refineParams.method & RefineParams::OTSU_ELLIPSE) {
                        ...
                        cv::threshold(image(blobNeighborhoodRoi), blobNeighborhood, 0, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
                        cv::findContours(blobNeighborhood, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE, ul);

                        if (contours.size() == 1) {
                            cv::RotatedRect fit;
                            if (refineParams.method & RefineParams::ELLIPSE_AMS) {
                                fit = cv::fitEllipseAMS(contours[0]);
                            } else if (refineParams.method & RefineParams::ELLIPSE_DIRECT) {
...
void draw(cv::Mat &image, const tbb::concurrent_vector<Detection> &detections, float drawScale, int thickness) {
    ...
    for (const auto &d : detections) {
        ...
        cv::putText(image, 
            ...
            1/4.f * std::accumulate(d.arucoCorners.begin(), d.arucoCorners.end(), cv::Point2f(0, 0)),
